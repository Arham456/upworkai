...
function matchScoreBg(score: number) {
  if (score >= 8) return "bg-violet-500/10 border-violet-500/30";
      ...
      <ProBlur isPro={isPro} featureName="Fear Detection">
        <div className="rounded-xl border border-violet-500/20 bg-violet-500/5 p-5 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="rounded-full bg-violet-500/15 border border-violet-500/30 px-3 py-1 text-xs font-semibold text-violet-300">
              {fearLabel} Fear
            </span>
          </div>
          ...
          <div className="space-y-2">
            <p className="text-xs font-medium text-zinc-500 uppercase tracking-wide">Top Opening Lines</p>
            <ol className="space-y-2">
              {data.openingLines.slice(0, 3).map((line, i) => (
                <li key={i} className="flex items-start gap-2.5 text-sm text-zinc-300">
                  <span className="shrink-0 flex h-5 w-5 items-center justify-center rounded-full bg-violet-500/15 border border-violet-500/25 text-xs font-bold text-violet-400">
                    {i + 1}
                  </span>
                  {line}
                </li>
...
function ConnectRoiCard({
  ...
  const verdictColors = {
    ...
    Priority: "bg-violet-500/20 text-violet-300 border-violet-500/40",
  };
    ...
    <motion.div variants={fadeUp}>
      <ProBlur isPro={isPro} featureName="Connect ROI Score">
        <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-5 space-y-4">
          <div className="flex items-center gap-2 text-xs font-medium text-amber-400 uppercase tracking-wide">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="rounded-lg bg-zinc-800/50 px-3 py-2.5 space-y-0.5">
              ...
              <p className="text-lg font-bold text-amber-400">{display.winProb}%</p>
            </div>
            {display.expectedEarnings > 0 && (
              <div className="rounded-lg bg-zinc-800/50 px-3 py-2.5 space-y-0.5">
                <p className="text-[10px] font-medium text-zinc-500 uppercase tracking-wide">Expected</p>
                <p className="text-lg font-bold text-amber-400">${display.expectedEarnings.toLocaleString()}</p>
              </div>
            )}
            <div className="rounded-lg bg-zinc-800/50 px-3 py-2.5 space-y-0.5">
              <p className="text-[10px] font-medium text-zinc-500 uppercase tracking-wide">ROI/connect</p>
              <p className="text-lg font-bold text-amber-400">${display.roiScore}</p>
            </div>
          </div>
    ...
    <div className="flex h-screen bg-[#0a0a0a] overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto pt-14 md:pt-0">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10 space-y-8">
          <div>
            <h1 className="text-2xl font-bold text-white tracking-tight">Analyze Job</h1>
          ...
          <div className="rounded-xl border border-zinc-800 bg-[#111111] p-6 space-y-4">
            {/* Mode toggle */}
            <div className="flex items-center gap-1 rounded-lg bg-[#0a0a0a] border border-zinc-800 p-1 w-fit">
              <button
              ...
              >
                ...
                Paste full job page
                <span className="rounded-full bg-violet-500/15 border border-violet-500/25 text-violet-400 px-1.5 py-0 text-[10px] font-semibold">
                  More intel
                </span>
              </button>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <input
                ...
                className="w-20 rounded-lg border border-zinc-800 bg-[#0a0a0a] px-3 py-1.5 text-sm text-white text-center focus:outline-none focus:border-violet-500 transition-colors"
              />
            </div>
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-lg bg-violet-600 hover:bg-violet-700 px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              {loading ? (
                <>
            ...
            {result && (
              ...
              >
                ...
                <motion.div variants={fadeUp} className="grid grid-cols-2 gap-4">
                  ...
                  <div className="rounded-xl border border-zinc-800 bg-[#111111] p-5 flex flex-col justify-center gap-2">
                ...
                <motion.div
                  variants={fadeUp}
                  className="rounded-xl border border-zinc-800 bg-[#111111] p-5 space-y-2"
                >
                  <div className="flex items-center gap-2 text-xs font-medium text-zinc-400 uppercase tracking-wide">
                    <FileText className="w-3.5 h-3.5" />
                    Job Summary
                ...
                <motion.div
                  variants={fadeUp}
                  className="rounded-xl border border-violet-500/20 bg-[#111111] p-5 space-y-2 border-l-[3px] border-l-violet-500"
                >
                  <div className="flex items-center gap-2 text-xs font-medium text-zinc-400 uppercase tracking-wide">
                    <TrendingUp className="w-3.5 h-3.5" />
                    Client&apos;s Core Fear
                ...
                {result.redFlags.length === 0 && (
                  ...
                  >
                    ...
                    <p className="text-sm text-emerald-300">No red flags detected — looks like a clean posting.</p>
                  </motion.div>
                )}
